...
class ExploreListViewHolder(
    itemView: View,
    private val listener: (image: ImageView, item: ExploreItem) -> Unit
    ...
    private val adapter: BasicSrvAdapter by lazy {
        BasicSrvAdapter().apply {
            addViewHolder(ExploreItem::class, R.layout.item_explore_place) { ExploreItemViewHolder(it, listener) }
        }
    }
...
class ExploreItemViewHolder(
    itemView: View,
    private val listener: (image: ImageView, item: ExploreItem) -> Unit
) : RecyclerView.ViewHolder(itemView), SrvViewHolder<ExploreItem> {
